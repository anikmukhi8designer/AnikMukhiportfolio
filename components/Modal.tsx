      ...
      {project && (
        ...
        >
            ...
            <div className="fixed top-0 left-0 right-0 z-[70] p-6 flex justify-between items-start pointer-events-none">
                <button 
                    onClick={onClose}
                    className="pointer-events-auto flex items-center gap-2 px-5 py-2.5 bg-background border border-border rounded-full hover:bg-foreground hover:text-background transition-all duration-300 group shadow-lg"
                >
                ...
                <button 
                    onClick={onClose}
                    className="pointer-events-auto p-3 bg-background border border-border rounded-full hover:bg-red-500 hover:border-red-500 hover:text-white transition-all duration-300 shadow-lg"
                >
                    <X className="w-5 h-5" />
                </button>
            </div>
            ...
            <div className="relative w-full h-[75vh] md:h-[85vh] overflow-hidden bg-muted flex items-center justify-center">
                ...
                <motion.div 
                    className="relative z-10 w-full px-6 md:px-12 lg:px-24 mt-24"
                    style={{ y: textY }}
                >
                    <div className="max-w-screen-2xl mx-auto flex flex-col items-start gap-4">
                ...
                <div className="border-b border-border">
                    <div className="max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-border">
                        <div className="p-10 lg:p-14">
                            <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Client</span>
                            <span className="text-2xl font-bold">{project.client}</span>
                        </div>
                        <div className="p-10 lg:p-14">
                            <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Year</span>
                            <span className="text-2xl font-bold font-mono">{project.year}</span>
                        </div>
                        <div className="p-10 lg:p-14">
                            <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Services</span>
                            <div className="flex flex-col gap-1">
                                {project.roles.map(role => (
                                    <span key={role} className="text-sm font-bold">{role}</span>
                                ))}
                            </div>
                        </div>
                        <div className="p-10 lg:p-14 flex flex-col justify-center gap-6 bg-secondary/10">
                            {project.link && (
                                <a href={project.link} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest hover:text-primary transition-colors">
                                    [ Visit Site <ArrowUpRight className="w-4 h-4" /> ]
                                </a>
                            )}
                            {project.githubRepoUrl && (
                                <a href={project.githubRepoUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest hover:text-primary transition-colors">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12">
                            ...
                            <div className="lg:col-span-10">
                                <div className="p-8 md:p-24 border-b border-border">
                                    <h3 className="text-4xl md:text-6xl font-bold leading-tight tracking-tight max-w-5xl mb-16">
                                        {project.description}
                                    </h3>
                                ...
                                <div className="space-y-0">
                                    {project.images?.map((img, idx) => (
                                        <div key={idx} className="bg-muted border-b border-border overflow-hidden group">
                                            <img 
                ...
                <div className="bg-background border-t border-border mt-0 relative z-20">
                    <div className="max-w-screen-2xl mx-auto px-6 py-24">
                        <div className="flex justify-between items-end mb-16">
                            <h3 className="text-4xl md:text-7xl font-bold tracking-tighter">Next Projects</h3>
                            <button onClick={onClose} className="hidden md:block text-xs font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors">
                                [ All Projects ]
                            </button>
                        </div>
                        ...
                        <div className="max-w-screen-2xl mx-auto px-6">
                            ...
                            <div className="flex flex-col justify-end items-start md:items-end gap-6">
                                ...
                                <a 
                                    ...
                                    href={social.url} 
                                    className="text-sm text-muted-foreground hover:text-primary transition-colors uppercase tracking-widest font-bold" 
                                    target="_blank" 
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center text-[10px] uppercase tracking-widest text-muted-foreground border-t border-border pt-8">
                            <p>&copy; {new Date().getFullYear()} Mukhi Anik. All rights reserved.</p>
                            <p>Product Design Portfolio 2024</p>
                            </div>
                        </div>
                    </footer>
                </div>
