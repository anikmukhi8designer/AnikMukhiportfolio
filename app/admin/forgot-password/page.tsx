      ...
      return (
        <div className="min-h-screen flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-white p-8 rounded-2xl border border-neutral-200 text-center">
                ...
                <p className="text-neutral-500 mb-6">If an account exists, a Super Admin will review your request.</p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white p-8 rounded-2xl border border-neutral-200 shadow-xl">
            <Link href="/admin/login" className="inline-flex items-center gap-2 text-sm text-neutral-500 hover:text-neutral-900 mb-6"><ArrowLeft className="w-4 h-4"/> Back</Link>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <textarea value={reason} onChange={(e) => setReason(e.target.value)} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-neutral-900 outline-none" placeholder="e.g. Locked out"/>
                </div>
                <button disabled={status === 'loading'} className="w-full py-3 bg-neutral-900 text-white font-bold rounded-lg hover:bg-black transition-colors flex items-center justify-center gap-2">
                    {status === 'loading' ? <Loader2 className="w-4 h-4 animate-spin"/> : <Send className="w-4 h-4" />} Submit Request
                </button>
            </form>
        </div>
    </div>
