...
import { UserPlus, Shield, Clock, Check, X, Loader2, Copy } from 'lucide-react';
...
export default function SuperAdminDashboard() {
  ...
  const handleRequestAction = async (id: string, action: 'approve' | 'reject') => {
    const res = await fetch('/api/admin/reset-action', {
        method: 'POST',
    ...
    if(data.success) {
        ...
        if(data.temp_password) {
            setCreatedTempPass(data.temp_password); // Reuse this modal for reset output
        }
        fetchData();
    } else {
        toast.error(data.error);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-12">
      ...
      {/* 1. Reset Requests Section */}
      <section>
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <Clock className="w-5 h-5 text-orange-500" /> Pending Reset Requests
        </h2>
        {requests.length === 0 ? (
            <div className="p-6 bg-white rounded-xl border border-neutral-200 text-neutral-400 text-sm">No pending requests.</div>
        ) : (
                ...
                {requests.map(req => (
                    <div key={req.id} className="bg-white p-4 rounded-xl border border-neutral-200 shadow-sm flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => handleRequestAction(req.id, 'reject')} className="px-3 py-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded flex items-center gap-1">
                                <X className="w-3 h-3"/> Reject
                            </button>
                            <button onClick={() => handleRequestAction(req.id, 'approve')} className="px-3 py-1.5 text-xs font-bold text-green-600 bg-green-50 hover:bg-green-100 rounded flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                <Shield className="w-5 h-5 text-blue-500" /> System Users
            </h2>
                    ...
                    <tbody className="divide-y divide-neutral-100">
                        {users.map(u => (
                            <tr key={u.id}>
                                ...
                                <td className="px-4 py-3">
                                    {u.is_temp_password ? (
                                        <span className="bg-orange-100 text-orange-700 px-2 py-0.5 rounded text-[10px] font-bold">Temp PW</span>
                                    ) : (
        ...
        <div className="bg-white p-6 rounded-xl border border-neutral-200 h-fit">
            ...
            <form onSubmit={handleCreateUser} className="space-y-4">
                ...
                <button className="w-full bg-neutral-900 text-white py-2 rounded text-sm font-bold hover:bg-black">
                    Create User
                </button>
            </form>
        </div>
      ...
      {createdTempPass && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white p-8 rounded-2xl max-w-sm w-full text-center">
                ...
                <div className="bg-neutral-100 p-4 rounded-lg font-mono text-lg font-bold tracking-widest break-all mb-6 relative group cursor-pointer"
                     onClick={() => { navigator.clipboard.writeText(createdTempPass); toast.success("Copied"); }}>
                    {createdTempPass}
                    <Copy className="w-4 h-4 absolute top-2 right-2 opacity-50"/>
                </div>
                <button 
                    onClick={() => setCreatedTempPass(null)}
                    className="w-full bg-neutral-900 text-white py-2 rounded font-bold"
                >
                    Done
                </button>
            </div>
